...
import { commentEditLink, check } from '@wordpress/icons';

import { getIcon } from '../utils';

const ListItem = ({ id, platform, meta, updateMetaValue, isPro }) => {
	...
	return (
		<VStack>
			<HStack alignment="center" justify="flex-start" spacing={2}>
				...
				<HStack>
					...
					<Button
						...
						label={
							isEditing
								? ropApiSettings.labels.settings.save
								: ropApiSettings.labels.publish_now.edit_message
						}
						showTooltip={true}
						disabled={
							!Object.keys(
			...
			{isEditing && (
				<TextareaControl
					label={
						ropApiSettings.labels.publish_now.custom_share_message
					}
					placeholder={
						ropApiSettings.labels.publish_now
							.custom_share_message_placeholder
					}
					help={
						<p
							dangerouslySetInnerHTML={{
								__html: ropApiSettings.labels.publish_now
									.custom_instant_share_messages_upsell,
							}}
						/>
					}
					value={socialMessage}
