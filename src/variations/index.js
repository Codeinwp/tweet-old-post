...
const Image = ({ id } ) => {
    const image = useSelect(
        ( select ) =>
            id ? select( coreStore ).getMedia( id ) : null,
        [ id ]
    );
...
const Variations = ({
    ...
    return (
        <PanelBody
            ...
            initialOpen={ false }
        >
            <p dangerouslySetInnerHTML={{ __html: ropApiSettings.labels.post_editor.custom_message_info }} />

            { meta.rop_custom_messages_group?.map( ({ rop_custom_description }, index ) => (
                <Spacer paddingY={ 2 }>
                    <Card isRounded={ false }>
                        <CardBody>
                            ...
                            <VStack>
                                <Image id={ meta.rop_custom_images_group?.[ index ]?.rop_custom_image } />
                                ...
                                >
                                    ...
                                    { meta.rop_custom_images_group?.[ index ]?.rop_custom_image && (
                                        <Button
                                            variant="secondary"
                                            onClick={ () => removeImage( index ) }
                                        >
                                            { ropApiSettings.labels.post_editor.variation_remove_image }
                                        </Button>
                                    ) }
                                </HStack>
                            </VStack>
                            ...
                            { index > 0 && (
                                ...
                                >
                                    { ropApiSettings.labels.post_editor.remove_variation }
                                </Button>
                            ) }
                        </CardBody>
                    </Card>
