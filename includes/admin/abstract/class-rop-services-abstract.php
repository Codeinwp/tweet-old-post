...
abstract class Rop_Services_Abstract {
	...
	/**
	 * Delete Revive Network Feed post
	 *
	 * @param string $given_id Social media ID.
	 * @param bool   $reverse replace underscore or put it back.
	 *
	 ...
	 * @since 8.6.1
	 */	
	protected function rop_is_revive_network_share( $post_id ){
	 ...
	 * @since 8.6.1
	 */	
	protected function rop_prepare_revive_network_share( $post_details ){
	 ...
	 * @since 8.6.1
	 */	
	protected function rop_delete_revive_network_feed_post( $post_id, $service_id ){

		$services_model = new Rop_Services_Model;
		
		$shared_to_accounts = get_post_meta( $post_id, 'revive-network-shared-to-accounts', true );
 
		
		update_option('rop_rn_shared_to_accounts', print_r($shared_to_accounts, true));
		
		
		if( !empty($shared_to_accounts) ){
			
			$active_accounts = array_keys($services_model->get_active_accounts());
			$active_account_count = count($active_accounts);

			$x = 0;
			
			foreach( $shared_to_accounts as $account ){
				// if the post has been shared for this account in the loop
				// increment x by 1
			...
			update_option('rop_rn_x', print_r($x, true));

			if( $x >= $active_account_count ){

				$deleted = wp_delete_post( $post_id, true);
				update_option('rop_rn_deleted_rn_post', print_r($deleted, true));
				if(empty($deleted)){
				$this->logger->alert_error( 'Failed to delete post' );
				return false;
				}
		...
		if( empty($shared_to_accounts) ){
			$shared_to_accounts = array();
		}

		$shared_to_accounts[] = $service_id;

		update_post_meta( $post_id, 'revive-network-shared-to-accounts', $shared_to_accounts );


		// // get the posts buffer containing the past shares
		// $posts_buffer_accounts = get_option('rop_data')['posts_buffer'];
		// $x = 0;
		// // $total_accounts = count($posts_buffer_accounts);
		// $active_account_count = count($services_model->get_active_accounts());

		// foreach( $posts_buffer_accounts as $account_post_buffer ){
		// 	// if the post has been shared for this account in the loop
		// 	// increment x by 1
		// 	if( in_array( $post_id, $account_post_buffer ) ){
		// 		$x++;
		// 	}
		// }
		// update_option('rop_rn_x', print_r($x, true));
		// update_option('rop_rn_active_account_count', print_r($active_account_count, true));
